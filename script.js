...
function aplicarDesconto() {
  ...
  totalElem.textContent = Total: R$ ${novoTotal.toFixed(2)} (com desconto 🎉);
}
...
if (botaoCupom) {
  botaoCupom.addEventListener("click", () => {
    ...
    if (jaUsouCupomHoje()) {
      msgCupom.textContent = "⚠️ Você já usou um cupom hoje. Tente novamente amanhã.";
    ...
    if (codigo === CUPOM_VALIDO) {
      aplicarDesconto();
      salvarUsoCupom();
      msgCupom.textContent = "✅ Cupom aplicado com sucesso!";
...
// Função para aplicar o desconto, se existir
function aplicarCupom() {
  ...
  infoCupom.innerHTML = `
    🎉 Cupom aplicado: <strong>${cupomSalvo.codigo}</strong> (${desconto}% off)<br>
